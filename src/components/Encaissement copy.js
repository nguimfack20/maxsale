...
class Encaissement extends React.Component {
  ...
  constructor(props) {
    ...
    this.getAllMotif = this.getAllMotif.bind(this);
  ...
  getAllMotif = async () => {
    let res = axios.get('http://maxsalesbackend.com/api/motifAll')
...
/*     console.warn(str2);
    ...
    console.warn(item.matricule);
     ...
     let result = await fetch('http://maxsalesbackend.com/api/saveencaissement', {
      method: 'POST',
      headers: {
        'Content-type': 'application/json',
        'Accept': 'application/json'
     ...
     // body: JSON.stringify(item)
       body: JSON.stringify({codedepot, motif, modeencai, montant, matricule,
        description}) 
    });
    ...
    return (
      <div>
        ...
        <div className="topuser">
          <img style={{ width: '30px', height: '30px' }} src={"encaissement.png"} alt="produit" />
          <img style={{ width: '30px', height: '30px' }} src={"encaissement.png"} alt="produit" />
          ENCAISSEMENT
        </div>
        ...
        <div className="utiGau">
          ...
          <div style={{ textAlign: 'center', fontWeight: 'bold', color: '#135D74' }}>Enrégistrez un Encaissement</div>
          ...
          <div style={{ float: 'left', marginTop: '5px', width: '180px' }}>
            ...
            <div style={{ marginTop: '-22px' }}>
              <select onClick={() => this.getAllMotif(this.state.motif)} style={{ marginLeft: '165px', width: '150px', border: ' solid #D1D7DC', fontSize: '11px' }} value={this.state.motif}
            ...
            <div style={{ marginTop: '-20px' }}>
              <select onClick={() => this.getAllmodeencaissement(this.state.modeencai)} style={{ marginLeft: '165px', width: '150px', border: ' solid #D1D7DC', fontSize: '11px' }} value={this.state.modeencai}
            ...
            <textarea value={this.state.description ? this.state.description : this.state.description} style={{ fontSize: '12px', marginTop: '-27px', width: '150px', marginLeft: '165px' }}  type="textarea"
