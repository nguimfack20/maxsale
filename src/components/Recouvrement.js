...
class Recouvrement extends React.Component {
  ...
  OperationInfo = async (id) => {
    this.state.notedebittab.forEach(element => {
      if (element.id === id) {
        ...
        this.setState({ personnel: element.personnel })
        this.setState({ notedebitID: id })
        this.setState({ code: "recouvrement" })
        this.setState({ montant: element.reste })
  ...
  ajouter = async () => {
    ...
    let notedebitID = this.state.notedebitID;
    let personnel = this.state.personnel;
    let description = this.state.description;
    let item = {personnel, montant, description ,personnel,notedebitID}


    console.warn(item);
    //console.warn(item.matricule);

    let result = await fetch('http://maxsalesbackend.com/api/recouvrementadd', {
      method: 'POST',
      headers: {
    ...
    return (
      <div>
        ...
        <div className="topuser">
          <img style={{ width: '30px', height: '30px' }} src={"recou.png"} alt="produit" />
         <span style={{margin:'15px'}}>Recouvrement</span> 
        </div>

        <div className="utiGauNotedebit">


          <div style={{ textAlign: 'center', fontWeight: 'bold', color: '#135D74' }}>Régularisez Une Sanction</div>
          ...
          <div style={{ float: 'left', marginTop: '5px', width: '180px' }}>
            ...
            <div style={{ marginTop: '-20px' }}>
              <input disabled onClick={() => this.getAllMatricule(this.state.personnel)} style={{ marginLeft: '165px', width: '150px',height:'40px', border: ' solid #D1D7DC', fontSize: '11px' }} value={this.state.personnel}
    ...
    {/*         <label style={{ marginTop: '15px', marginLeft: '-89px' }}>Nom </label>
            ...
            <div style={{ marginTop: '-20px' }}>
              ...
              style={{ marginLeft: '165px', width: '150px',height:'40px',
               border: ' solid #D1D7DC', fontSize: '11px' }} value={this.state.code}/>
        ...
        <div className="utiDroiNotedebit">
            ...
            <h5 style={{ textAlign: 'center', fontWeight: 'bold', color: '#135D74' }}> Liste de débits</h5>
